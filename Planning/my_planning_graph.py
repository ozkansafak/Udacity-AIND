...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            new = PgNode_a(a)
            if new.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(new)

        # Now, connect s and a nodes
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            ...
            for node_s in node_a.effnodes:
                s_nodes.add(node_s)
                # add node_a to state node
                # Add state level to action, and action to state level
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check "positive effects" of node_a1. 
        # Make sure no conflicts happen with "negative effects" of node_a2 
    ...
    def h_levelsum(self) -> int:
        ...
        # determine level cost for each goal in problem
        # and add all them up
        level_sum = 0
        for goal in self.problem.goal:
            level_sum += cost(goal)
